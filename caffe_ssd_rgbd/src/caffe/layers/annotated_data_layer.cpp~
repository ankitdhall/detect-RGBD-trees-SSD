...
void AnnotatedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  /*for (std::vector<int>::const_iterator i = top_shape.begin(); i != top_shape.end(); ++i)
    LOG(INFO) << *i << ' ';
	*/
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
	...
	//LOG(INFO) << "at line 161 channels are = " << anno_datum.datum().channels();
    if (transform_param.has_distort_param()) {
		//LOG(INFO) << "has distort param";
      distort_datum.CopyFrom(anno_datum);
      this->data_transformer_->DistortImage(anno_datum.datum(),
                                            distort_datum.mutable_datum());
	  //LOG(INFO) << "at line 167 channels are = " << distort_datum.mutable_datum()->channels();
      if (transform_param.has_expand_param()) {
		//LOG(INFO) << "has expand param";
        expand_datum = new AnnotatedDatum();
        this->data_transformer_->ExpandImage(distort_datum, expand_datum);
		//LOG(INFO) << "at line 172 channels are = " << expand_datum->mutable_datum()->channels();
      } else {
        expand_datum = &distort_datum;
		//LOG(INFO) << "at line 175 channels are = " << expand_datum->mutable_datum()->channels();
      }
    } else {
      if (transform_param.has_expand_param()) {
		//LOG(INFO) << "has only expand param";
        expand_datum = new AnnotatedDatum();
        this->data_transformer_->ExpandImage(anno_datum, expand_datum);
		//LOG(INFO) << "at line 182 channels are = " << expand_datum->mutable_datum()->channels();
      } else {
		//expanded_anno_datum->mutable_datum()
		/*cv::Mat cv_img;
		cv_img = DecodeDatumToCVMat(anno_datum.datum(), -1);
		LOG(INFO) << "channels after decoding = " << cv_img.channels();
		// Expand the image.
		// Save the image into datum.
		EncodeCVMatToDatum(cv_img, "png", expand_datum->mutable_datum());
		expand_datum->mutable_datum()->set_label(anno_datum.datum().label());*/


        expand_datum = &anno_datum;
		//LOG(INFO) <<  anno_datum.datum().channels();
		//LOG(INFO) << "at line 196 channels are = " << expand_datum->mutable_datum()->channels();
      }
    }
    ...
    if (batch_samplers_.size() > 0) {
      ...
      if (sampled_bboxes.size() > 0) {
		...
		//LOG(INFO) << "at line 215 channels are = " << expand_datum->mutable_datum()->channels();
      }
    } else {
      sampled_datum = expand_datum;
		//LOG(INFO) << "at line 219 channels are = " << expand_datum->mutable_datum()->channels();
    }
	...
	//LOG(INFO) << "after all the transform preliminary";
    /*for (std::vector<int>::const_iterator i = top_shape.begin(); i != top_shape.end(); ++i)
    LOG(INFO) << *i << ' ';
	for (std::vector<int>::const_iterator i = shape.begin(); i != shape.end(); ++i)
    LOG(INFO) << *i << ' ';
	*/
    ...
    if (transform_param.has_resize_param()) {
      if (transform_param.resize_param().resize_mode() ==
		...
		//LOG(INFO) << top_shape.begin() << top_shape.begin() + 1 << top_shape.begin() + 4 << shape.begin() + 1;
