...
void DataTransformer<Dtype>::Transform(const Datum& datum,
                                       ...
                                       bool* do_mirror) {
  //LOG(INFO) << "in Transform at line 55";
...
void DataTransformer<Dtype>::Transform(const Datum& datum,
                                       ...
                                       NormalizedBBox* crop_bbox,
                                       bool* do_mirror) {
	//LOG(INFO) << "in Transform at line 160";
  ...
  if (datum.encoded()) {
	...
	cv_img = DecodeDatumToCVMat(datum, -1);
...
void DataTransformer<Dtype>::CropImage(const Datum& datum,
  ...
  if (datum.encoded()) {
    ...
    // Crop the image.
    cv::Mat crop_img;
	//LOG(INFO) << "in CropImage channels are = " << cv_img.channels();
    CropImage(cv_img, bbox, &crop_img);
	//LOG(INFO) << "in after CropImage channels are = " << crop_img.channels();
...
void DataTransformer<Dtype>::ExpandImage(const AnnotatedDatum& anno_datum,
  ...
  //LOG(ERROR) << "isEncoded? = " << anno_datum.datum().encoded();
  //cv::Mat cv_img;
...
void DataTransformer<Dtype>::DistortImage(const Datum& datum,
  ...
  if (datum.encoded()) {
	...
	cv_img = DecodeDatumToCVMat(datum, -1);
	//LOG(INFO) << "inside DistortImage channels = " << cv_img.channels();
    // Distort the image.
    cv::Mat distort_img = ApplyDistort(cv_img, param_.distort_param());
    // Save the image into datum.
	//LOG(INFO) << "inside after DistortImage channels = " << distort_img.channels();
...
void DataTransformer<Dtype>::ExpandImage(const cv::Mat& img,
  ...
  //LOG(INFO) <<  "Channels in data_transformer = " << img_channels;
