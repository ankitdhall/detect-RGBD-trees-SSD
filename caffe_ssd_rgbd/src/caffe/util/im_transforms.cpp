...
void RandomBrightness(const cv::Mat& in_img, cv::Mat* out_img,
  ...
  if (prob < brightness_prob) {
	...
	// create BGR image to adjust
	vector<cv::Mat> channels2;
	channels2.push_back(channels[1]);
	channels2.push_back(channels[2]);
	channels2.push_back(channels[3]);
	cv::Mat adjust_out;
	cv::merge(channels2, adjust_out);

	// adjust brightness of BGR image
	AdjustBrightness(adjust_out, delta, out_img);
	//LOG(INFO) << "after brightness adjustment: " << out_img->channels();
	
	// combine D+BGR to get 4 channels again
	
	channels2.clear();
	cv::split(*out_img, channels2);
	channels[1]=channels2[0];
	channels[2]=channels2[1];
	channels[3]=channels2[2];
...
void RandomContrast(const cv::Mat& in_img, cv::Mat* out_img,
  ...
  float prob;
  //LOG(INFO) << "RandomContrast" << in_img.channels();
  caffe_rng_uniform(1, 0.f, 1.f, &prob);
  if (prob < contrast_prob) {
	...
	// adjust contrast of BGR image
	AdjustContrast(adjust_out, delta, out_img);
	//LOG(INFO) << "after contrast adjustment: " << out_img->channels();
	
	// combine D+BGR to get 4 channels again
	/*cv::imshow("adjusted", *out_img);
	cv::imshow("disp", channels[0]);
	cv::waitKey(500);*/
	channels2.clear();
	cv::split(*out_img, channels2);
	channels[1]=channels2[0];
...
void RandomSaturation(const cv::Mat& in_img, cv::Mat* out_img,
  ...
  if (prob < saturation_prob) {
	...
	// adjust saturation of BGR image
	AdjustSaturation(adjust_out, delta, out_img);
	//LOG(INFO) << "after saturation adjustment: " << out_img->channels();
...
void RandomHue(const cv::Mat& in_img, cv::Mat* out_img,
  ...
  if (prob < hue_prob) {
	...
	// adjust hue of BGR image
	AdjustHue(in_img, delta, out_img);
	//LOG(INFO) << "after hue adjustment: " << out_img->channels();
